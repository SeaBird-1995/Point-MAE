...
Date: 2022-12-07 15:27:41
Email: haimingzhang@link.cuhk.edu.cn
Description: The loss functions for PU.
'''
...
class ChamferLoss(torch.nn.Module):
    ...
    def forward(self, pred, gt, radius=None, p_norm=2):
        ...
        if p_norm != 2:
            _, result_i, _, result2_i = NmDistanceFunction.apply(pred, gt)
            return result_i, result2_i
        else:
        ...
        CD_dist = 0.5 * pred2gt + 0.5 * gt2pred
        if radius is not None:
            cd_loss = CD_dist / radius
        cd_loss = torch.mean(CD_dist)
        return cd_loss


class CD_dist(torch.nn.Module):
    """
    CD_dist. bidirectional nearest neighbor distance of two point sets.
    """

    def __init__(self, threshold=None):
        super(CD_dist, self).__init__()
        # only consider distance smaller than threshold*mean(distance) (remove outlier)
        self.__threshold = threshold
